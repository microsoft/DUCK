...
type DBPlugin interface {
	...
	NewUser(id string, entry string) (eid string, err error)
	...
	NewDocument(id string, entry string) (eid string, err error)
	UpdateDocument(id string, entry string) (eid string, err error)
