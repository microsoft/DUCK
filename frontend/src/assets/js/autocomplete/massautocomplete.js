...
angular.module('MassAutoComplete', [])
    .directive('massAutocomplete', ["$timeout", "$window", "$document", "$q", function ($timeout, $window, $document, $q) {
        ...
        return {
            ...
            '<ul class="ac-menu">' +
            '<li ng-click="apply_selection($index)" ng-repeat="result in results" ng-if="$index > 0" ' +
                            ...
                            case KEYS.DOWN:
                                if ($scope.results.length >= 0) {
                                    if ($scope.show_autocomplete) {
                                        ...
                                        previous_value === null || previous_value === "" ? _suggest("  ", current_element) : _suggest(previous_value, current_element);
                                        $scope.show_autocomplete = true;
                                        $scope.selected_index = 0;
                                    }
                                    $scope.$apply();
