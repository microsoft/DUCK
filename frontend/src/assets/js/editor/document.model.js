 ...
 */
editorModule.service("DocumentModel", function(
  CurrentUser,
  TaxonomyService,
  GlobalDictionary,
  ...
  this.initialize = function(documentId) {
    return $q(function(resolve) {
      DataUseDocumentService.getDocument(documentId).then(function(
        useDocument
      ) {
        ...
        TaxonomyService.activate([
          GlobalDictionary.getDictionary(),
          context.document.dictionary.values()
        ]);
  ...
  this.reCalculateCodes = function() {
    context.document.statements.forEach(function(statement) {
      statement.useScopeCode = TaxonomyService.findCode(
        "scope",
        statement.useScope,
        context.document.locale,
        statement.useScope
      );
      statement.qualifierCode = TaxonomyService.findCode(
        "qualifier",
        statement.qualifier,
        context.document.locale,
        statement.qualifier
      );
      statement.dataCategoryCode = TaxonomyService.findCode(
        "dataCategory",
        statement.dataCategory,
        context.document.locale,
        statement.dataCategory
      );
      statement.sourceScopeCode = TaxonomyService.findCode(
        "scope",
        statement.sourceScope,
        context.document.locale,
        statement.sourceScope
      );
      statement.actionCode = TaxonomyService.findCode(
        "action",
        statement.action,
        context.document.locale,
        statement.action
      );
      statement.resultScopeCode = TaxonomyService.findCode(
        "scope",
        statement.resultScope,
        context.document.locale,
        statement.resultScope
      );
  ...
  this.lookupAndSetTerms = function(document) {
    document.statements.forEach(function(statement) {
      statement.useScope = TaxonomyService.findTerm(
        "scope",
        statement.useScopeCode,
        document.locale,
        statement.useScopeCode
      );
      statement.qualifier = TaxonomyService.findTerm(
        "qualifier",
        statement.qualifierCode,
        document.locale,
        statement.qualifierCode
      );
      statement.dataCategory = TaxonomyService.findTerm(
        "dataCategory",
        statement.dataCategoryCode,
        document.locale,
        statement.dataCategoryCode
      );
      statement.sourceScope = TaxonomyService.findTerm(
        "scope",
        statement.sourceScopeCode,
        document.locale,
        statement.sourceScopeCode
      );
      statement.action = TaxonomyService.findTerm(
        "action",
        statement.actionCode,
        document.locale,
        statement.actionCode
      );
      statement.resultScope = TaxonomyService.findTerm(
        "scope",
        statement.resultScopeCode,
        document.locale,
        statement.resultScopeCode
      );
      if (typeof statement.dataCategories !== "undefined") {
        statement.dataCategories.forEach(function(datacategory) {
          datacategory.qualifier = TaxonomyService.findTerm(
            "qualifier",
            datacategory.qualifierCode,
            document.locale,
            datacategory.qualifierCode
          );
          datacategory.dataCategory = TaxonomyService.findTerm(
            "dataCategory",
            datacategory.dataCategoryCode,
            document.locale,
            datacategory.dataCategoryCode
          );
        });
  ...
  this.complianceCheck = function() {
    return $q(function(resolve, reject) {
      ...
      // FIXME ruleset id
      DataUseDocumentService.complianceCheck(context.document, "123").then(
        function(complianceResult) {
          context.state = complianceResult.compliant;
          context.document.statements.forEach(function(statement) {
            var statementExplanation = complianceResult.map.get(
              statement.trackingId
            );
  ...
  this.prepareDataCategories = function() {
    context.document.statements.forEach(function(statement) {
      ...
      statement.dataCategories.forEach(function(category) {
        category.qualifierCode = TaxonomyService.findCode(
          "qualifier",
          category.qualifier,
          context.document.locale,
          category.qualifier
        );
        category.dataCategoryCode = TaxonomyService.findCode(
          "dataCategory",
    ...
    context.document.statements.forEach(function(statement) {
      ...
      if (!ObjectUtils.isEmptyString(statement.useScope)) {
        if (
          ...
          context.createError(
            statement.errors.useScope,
            "Use scope is not recognized",
            errorNumber
          );
          errorNumber++;
        }
      ...
      if (!ObjectUtils.isEmptyString(statement.qualifier)) {
        if (
          ...
          context.createError(
            statement.errors.qualifier,
            "Qualifier is not recognized",
            errorNumber
          );
          errorNumber++;
        }
      ...
      if (!ObjectUtils.isEmptyString(statement.dataCategory)) {
        ...
        context.createError(
          statement.errors.dataCategory,
          "Data category is not specified",
          errorNumber
        );
        errorNumber++;
      } else {
      ...
      if (!ObjectUtils.isEmptyString(statement.sourceScope)) {
        if (
          ...
          context.createError(
            statement.errors.sourceScope,
            "Source scope is not recognized",
            errorNumber
          );
          errorNumber++;
        }
      ...
      if (!ObjectUtils.isEmptyString(statement.resultScope)) {
        if (
          ...
          context.createError(
            statement.errors.resultScope,
            "Result scope is not recognized",
            errorNumber
          );
          errorNumber++;
        }
  ...
  this.emptyStatement = function(statement) {
    return (
      (ObjectUtils.isNull(statement.useScope) ||
        statement.useScope.trim().length === 0) &&
      (ObjectUtils.isNull(statement.action) ||
        statement.action.trim().length === 0)
    );
  };
